...
function validarMayusculas(str) {
    ...
    //mayúsculas o espacios en blanco (\s). Devuelve true si la cadena cumple con este criterio, y false e
    //n caso contrario.
...
let proxy = new Proxy({}, {
    ...
    set: function(target, property, value) {
        ...
        if (property === 'name') {
            ...
            // letras mayúsculas), se asigna el valor transformado a mayúsculas al objeto proxy. Si no 
            //pasa la validación, se lanza un error.
...
//Después de obtener la respuesta, se convierte la respuesta en formato JSON utilizando el método .json().
    ...
    .then(jsonData => {
        ...
        let usuariosConADSO = jsonData.users.filter(user => user.user.includes('ADSO'));
        // Procesar los usuarios
        let users = usuariosConADSO.map(user => new Proxy(user, proxy));
        console.log(users);
