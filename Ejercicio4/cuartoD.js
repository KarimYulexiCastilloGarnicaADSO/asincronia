...
async function cargarDatos() {
    ...
    try {
        ...
        //los nombres de los aprendices y sus repositorios públicos.
        let resultados = await Promise.all(aprendices.map(async user => {
            try {
                ...
                let repositoriosFiltrados = repositorios.filter(repo => !repo.private && repo.name.includes("JavaScript") && repo.name.length > 5);
